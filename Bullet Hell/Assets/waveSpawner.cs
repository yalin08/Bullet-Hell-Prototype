...
public class waveSpawner : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        var sutaparticle = sutazu.gameObject.GetComponent<ParticleSystem>().main;

        starparticle.startColor = currentStarColor;
        sutaparticle.startColor = currentStarColor;
        ...
        if (gameStarted&&gameStartsIn>0) {
            ...
            if (!isStarParticleActive)
            {
               starParticle.SetActive(true);
                ...
                GameObject.FindWithTag("Player").GetComponent<BulletSpawn>().enabled = true;
                GameObject.FindWithTag("Player").GetComponent<JoystickPlayerExample>().isGameStarted = true; 
        }
    ...
    void resetStars()
    {
        if (GameObject.FindWithTag("Player").GetComponent<PlayerStats>().isDestroyed == false)
        {
            Debug.Log("stars reset");
            Animator staranim = stars.GetComponentInChildren<Animator>();
            Animator star2anim = sutazu.GetComponentInChildren<Animator>();
    ...
    void Loop()
    {
        ...
        shipmodelChange shipchange = GameObject.FindWithTag("Player").GetComponent<shipmodelChange>();
